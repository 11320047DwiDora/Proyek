                ...
                <form id="form_input">
                    <div class="row mb-7">
                        ...
                        <div class="col-sm-12 col-md-6">
                            <div class="mb-10">
                                <label for="nama_course" class="required form-label">Nama Mata Pelajaran</label>
                                <input type="text" class="form-control form-control-solid" name="nama_course" value="{{$course->nama_course}}">
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <label class="required fs-6 fw-bold mb-2">Kelas</label>
                            <select data-control="select2" data-placeholder="Pilih Tingkat Kelas" id="room_id" name="room_id" class="form-select form-select-solid">
                                <option value="" selected disabled>Pilih Kelas</option>
                                @foreach ($rooms as $item)
                                <option value="{{$item->id}}" {{$item->id == $course->room_id ? 'selected' : ''}}>{{$item->nama_kelas}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-sm-12 col-md-6">
